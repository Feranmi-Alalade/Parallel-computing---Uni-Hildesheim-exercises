...
hadoop jar /opt/hadoop/share/hadoop/tools/lib/hadoop-streaming-3.3.6.jar \
  ...
  -mapper "python3 mapper_adjacency.py" \
  -reducer "python3 reducer_adjacency.py" \
  -file mapper_adjacency.py \
  -file reducer_adjacency.py

# Step 3: Fetch result back
hdfs dfs -cat $OUTPUT_HDFS_DIR/part-00000 > $CONTAINER_WORKDIR/output_adjacency.txt
echo "MapReduce job complete. Output saved to output_adjacency.txt"