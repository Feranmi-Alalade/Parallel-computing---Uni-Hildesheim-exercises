...
hadoop jar /opt/hadoop/share/hadoop/tools/lib/hadoop-streaming-3.3.6.jar \
  ...
  -mapper "python3 mapper_image.py" \
  -reducer "python3 reducer_image.py" \
  -file mapper_image.py \
  -file reducer_image.py

# Step 3: Fetch result back
hdfs dfs -cat $OUTPUT_HDFS_DIR/part-00000 > $CONTAINER_WORKDIR/output_grayscale.txt
echo "MapReduce job complete. Output saved to output_grayscale.txt"