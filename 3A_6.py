...
df = pd.read_csv(r"C:\Users\hp\Downloads\hierarical_clustering_data_6.csv", header =0)
data = df.values
...
clustered_dataset_cluster0 = np.zeros((len(data),3), dtype=np.float64)

for cluster in range(len(clusters)):
    if cluster == 0:
        clustered_dataset_cluster0[:, 0:2] = data[clusters[cluster],:]
        clustered_dataset_cluster0[:,2] = 0

print(clustered_dataset_cluster0)